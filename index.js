...
function loadAccounts() {
  try {
    ...
    if (accounts.length === 0) {
      throw new Error("No private keys found in wallet.txt");
    }
    addLog(`Loaded ${accounts.length} accounts from wallet.txt`, "success");
  } catch (error) {
        ...
        figlet.text("ANNISA", { font: "ANSI Shadow" }, (err, data) => {
          if (err) {
            ...
            const coloredLines = lines.map((line, index) => {
              return index < half
                ? `{center}{bold}{red-fg}${line}{/red-fg}{/bold}{/center}`
                : `{center}{bold}{white-fg}${line}{/white-fg}{/bold}{/center}`;
            });
